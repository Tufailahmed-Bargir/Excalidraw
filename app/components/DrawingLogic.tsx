...
export const useDrawingLogic = (
  ...
  const erase = (cursor: { x: number; y: number }) => {
    ...
    setPaths((prev) =>
      prev.filter((path) => !isPathNearPoint(path.d, cursor, eraserSize)),
    );
  ...
  const deleteSelection = () => {
    if (selection) {
      setPaths((prev) =>
        prev.filter((path) => !isPathInSelection(path.d, selection)),
      );
      setShapes((prev) =>
        prev.filter((shape) => !isShapeInSelection(shape, selection)),
      );
      setSelection(null);
    }
  };
  ...
  useEffect(() => {
    ...
    const handleMouseDown = (e: MouseEvent) => {
      ...
      if (tool === "pan") {
        ...
        const clickedShapeId = shapes.findIndex((shape) =>
          isPointInShape(shape, cursor),
        );
        if (clickedShapeId !== -1) {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      if (tool === "pan" && isPanning) {
        ...
        setCurrentPath(
          (prev) => `${prev} L${cursor.x - pan.x},${cursor.y - pan.y}`,
        );
      } else if (tool === "eraser") {
        erase({ x: cursor.x - pan.x, y: cursor.y - pan.y });
      }
